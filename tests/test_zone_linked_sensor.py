...
class TestZoneLinkedSensorInit:
    def test_name_and_icon(self):
        hass = _make_hass()
        sensor = ZoneLinkedSensor(hass, "switch.valve", "linked_valve", "mdi:valve", "uid_1")
        assert sensor._attr_translation_key == "linked_valve"
        assert sensor._attr_icon == "mdi:valve"

    def test_unique_id(self):
        hass = _make_hass()
        sensor = ZoneLinkedSensor(hass, "switch.valve", "linked_valve", "mdi:valve", "linked_valve_orto")
    ...
    def test_device_info_assigned(self):
        ...
        sensor = ZoneLinkedSensor(hass, "switch.valve", "linked_valve", "mdi:valve", "uid", device)
...
class TestZoneLinkedSensorValue:
    def test_valve_on_returns_open(self):
        ...
        sensor = ZoneLinkedSensor(hass, "switch.valve", "linked_valve", "mdi:valve", "uid")
        sensor.hass = hass
    ...
    def test_missing_entity_returns_none(self):
        hass = _make_hass({})
        sensor = ZoneLinkedSensor(hass, "switch.missing", "linked_valve", "mdi:valve", "uid")
        sensor.hass = hass
        assert sensor.native_value is None
